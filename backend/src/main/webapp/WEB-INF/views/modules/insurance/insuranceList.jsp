...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnGiveInsurance").click(function(){
				$("#searchForm").attr("action","${ctx}/insurance/giveInsuranceForm");
				$("#searchForm").submit();
			});
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出用户数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/insurance/export");
						$("#searchForm").submit();
					}
				},{buttonsFocus:1});
...
<form:form id="searchForm" modelAttribute="vo" action="${ctx}/insurance/insuranceList?" method="post" class="breadcrumb form-search ">
	...
	<ul class="ul-form">
		...
		<li><label></label>
			<input id="btnGiveInsurance" class="btn btn-primary" type="button" value="赠送保险通道"/>
		</li>
	...
	<c:forEach items="${page.list}" var="insurance">
		<tr>
			...
			<td>
				...
				<c:if test="${insurance.state eq '6'}">待支付</c:if>
			</td>
			<td>
				<c:if test="${insurance.source eq 'buy'}">购买</c:if>
				<c:if test="${insurance.source eq 'give'}">赠送</c:if>
			</td>
