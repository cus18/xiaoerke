...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var id = $("input:hidden[name='keyWordId']").val();
            $.ajax({
                type: "post",
                url: "${ctx}/sys/keyword/keywordAndillnessRelationData",
                data: {keyWordId: id},
                dataType: "json",
        ...
        function sub() {
            ...
            for (var i = 0; i < count_level1; i++) {        //一维长度为i,i为变量，可以根据实际情况改变
...
//                tArray[i] = new Array();    //声明二维，每一个一维数组里面的一个元素都是一个数组；
            ...
            var keyWordId = $("input:hidden[name='keyWordId']").val();
            //发送数据
            $.ajax({
                type: "post",
                url: "${ctx}/sys/keyword/keywordAndillnessRelationData",
                data: {tArray: tArray, keyWordId: keyWordId},
                dataType: "json",
                success: function () {
                    location.href="${ctx}/sys/keyword/findAllkeyword?flag="+'1';
                    alertx("保存成功！");
                }
...
<form:form id="inputForm" modelAttribute="SearchKeyword" method="post">
    <sys:message content="${message}"/>

    <input type="hidden" id="keyWordId" name="keyWordId" value=${SearchKeyword.id}>
                            ...
                            <div style="position: relative;">
                                ...
                                <label style="position:absolute;left:159px;top:-1px;">
                                    ...
                                    <label>全选</label>
                                        ...
                                        <%-- <input id="${illnessVo.id}0"
                                               name=${illnessVo.id}  onclick="allNoSelect('${illnessVo.level_1}');"
                                               type="radio">
                                        <label for="${illnessVo.id}0">不选</label> --%>
                                </label>
