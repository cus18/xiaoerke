...
//        然后插入。。。
    ...
    @RequestMapping(value = "/searchDoctorInfoByName", method = {RequestMethod.POST, RequestMethod.GET})
    public
    @ResponseBody
    Map<String, Object> searchDoctorInfoByName(HttpSession session, HttpServletRequest request, @RequestBody Map<String, Object> params) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
        String doctorName = (String) params.get("doctorName");
        List<ConsultDoctorInfoVo> doctorInfoVoList = nonRealTimeConsultUserService.getRecentTimeListByName(doctorName);
        resultMap.put("departmentVoList", doctorInfoVoList);
        resultMap.put("status", "success");
        return resultMap;
    }
}